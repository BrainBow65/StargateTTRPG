...
# possibly add gate address when creating new planets? need to check if already exists
...
def check_name_availability(name, folder_path):
    for filename in os.listdir(folder_path):
        if filename.startswith(name[:7]):
            return False
    return True

def generate_unique_name(folder_path):
    while True:
        name = generate_name()
        if check_name_availability(name, folder_path):
            return name

def create_planet_file(content, name, folder_path):
    file_name = name
...
class PlanetGeneratorTab(QWidget):
    ...
    def generate_planet(self):
        ...
        name = generate_name()
        while not check_name_availability(name, FOLDER_PATH):  # Pass FOLDER_PATH as the second argument
            name = generate_name()

        unique_name = generate_unique_name(FOLDER_PATH)  # Pass FOLDER_PATH as an argument
        markdown_content = f"The new planet's biome is {currentBiome}\nThe nearby geological features are: {currentGeoFeatures}"
        file_name = create_planet_file(markdown_content, unique_name, FOLDER_PATH)  # Pass FOLDER_PATH as an argument
...
class SocietyGeneratorTab(QWidget):
    ...
    def generate_society(self):
        currentTechLvl, currentGovType, currentCulture, Alien, currentRelig = self.generate_random_values()
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.settings = QSettings("RainbowElephantGaming", "StargateTTRPG")
        self.setWindowTitle("StargateTTRPG App")
        self.setGeometry(100, 100, 800, 600)

        tab_widget = QTabWidget()
        self.planet_tab = PlanetGeneratorTab()
        self.society_tab = SocietyGeneratorTab()
